...
#danganhphu trợ lý ảo Ralanhphu
import os
import playsound
...
#máy tính nhận dạng âm thanh của người đọc tối đa 3
def get_text():
    for i in range(3):
        text = get_audio()
        if text:
...
def send_email(text):
    ...
    if 'phú' in recipient:
        ...
        mail.ehlo()
        mail.starttls()
        mail.login('phu.da.61cntt@ntu.edu.vn', '12345678phu')
        mail.sendmail('phu.da.61cntt@ntu.edu.vn',
                      'phudanganh@gmail.com', content.encode('utf-8'))
        mail.close()
...
def change_wallpaper():
    ...
    urllib2.urlretrieve(photo, "C:/Users/anhphu/Downloads/test.png")
    image = os.path.join("C:/Users/anhphu/Downloads/a.png")
...
#Kết hợp tất cả chức năng Trợ lý ảo
def assistant():
    speak("Xin chào, bạn tên là gì nhỉ?")
    name = get_text()
    if name:
        speak("Chào bạn {}".format(name))
        speak("Bạn cần Bot Ralanhphu có thể giúp gì ạ?")
        while True:
            text = get_text()
            if not text:
                break
